...
class ReglesDebutMandat(ReglesInterface):
    ...
    def _etat_budgetaire(self, etat: "Etat") -> Tuple[int, int, int, int]:
        """
        Retourne:
          total_recettes: int
          total_depenses: int (incluant intérêts)
          interets: int
          variation_dette: int (+ => dette augmente ; - => dette diminue)
        """
        eco = getattr(etat, "eco", None)
        if eco is None:
            return 0, 0, 0, 0

        total_recettes = int(round(sum((getattr(p, "valeur", 0) or 0) for p in eco.recettes.values())))
        total_depenses_hors_interets = int(round(sum((getattr(p, "valeur", 0) or 0) for p in eco.depenses.values())))

        dette = float(getattr(eco, "dette", 0) or 0)
        taux = float(getattr(eco, "taux_interet", 0) or 0)
        interets = int(round(dette * taux))

        total_depenses = total_depenses_hors_interets + interets
        solde = total_recettes - total_depenses
        variation_dette = int(-solde)

        return total_recettes, total_depenses, interets, variation_dette


    def _regle_fin_tour(self, etat: "Etat") -> List["Command"]:
        cmds: List["Command"] = []

        total_recettes, total_depenses, interets, variation_dette = self._etat_budgetaire(etat)

        # données pour journal (dans ce scope)
        eco = getattr(etat, "eco", None)
        dette_avant = int(round(float(getattr(eco, "dette", 0) or 0))) if eco is not None else 0
        taux_interet = float(getattr(eco, "taux_interet", 0) or 0) if eco is not None else 0.0

        # appliquer variation de dette
        if variation_dette != 0:
            cmds.append({"op": "eco.delta_dette", "montant": int(variation_dette)})

        # solde = - variation_dette
        solde = int(-variation_dette)
        ...
        cmds.append({
            ...
            "payload": {
                ...
                "recettes": int(total_recettes),
                "depenses_hors_interets": int(total_depenses - interets),
                "interets": int(interets),
                "depenses_total": int(total_depenses),
                "solde": int(solde),                 # + surplus / - déficit
                "delta_dette": int(variation_dette), # + dette augmente
                "dette_avant": int(dette_avant),
                # si tu veux un % entier pour l'UI :
                "taux_interet_pct": int(round(taux_interet * 100)),
            }
        })

        cmds.append({"op": "journal", "payload": {"message": "Fin du tour."}})

        cmds.extend(self.condition_fin_partie(etat))
        if not etat.termine:
    ...
    def analyse_etat(self, etat: Etat) -> List[Command]:
        ...
        # score simple (tu peux pondérer plus tard)
        score_axes = int(social + eco + inst + env)

        # 2) calculer quelques agrégats simples
        total_recettes, total_depenses, interets, variation_dette = self._etat_budgetaire(etat)
        eco_obj = getattr(etat, "eco", None)
        dette = int(round(float(getattr(eco_obj, "dette", 0) or 0))) if eco_obj else 0
        taux = float(getattr(eco_obj, "taux_interet", 0) or 0) if eco_obj else 0.0

        solde = int(-variation_dette)  # + surplus / - déficit

        taux_interet_pct = int(round(taux * 100))
        interets_pct_recettes = int(round(100 * interets / max(1, total_recettes)))
        ...
        budget = {
            ...
            "interets": int(interets),
            "solde": int(solde),
            "delta_dette": int(variation_dette),
            "dette": int(dette),
            "taux_interet_pct": int(taux_interet_pct),
            "interets_pct_recettes": int(interets_pct_recettes),
        }

        indicateurs = {
            # axes existants
            "social": social, "economique": eco, "institutionnel": inst, "environnement": env,
            "score_axes": score_axes,
            # remplace l’ancien solde “hors intérêts”
            "solde": solde,
            # ajout structuré
            "budget": budget,
        }
        ...
        # lecture politique (opposition)
        cmds.append(self._journal_lecture_opposition(etat, indicateurs, missions_maj))
    ...
    def generer_missions_opposition(
        ...
        inst = indicateurs["institutionnel"]
        env = indicateurs["environnement"]
        score_axes = int(indicateurs.get("score_axes", 0))
        solde = int(indicateurs.get("solde", 0))

        budget = indicateurs.get("budget", {}) or {}
        interets_pct_recettes = int(budget.get("interets_pct_recettes", 0))
        delta_dette = int(budget.get("delta_dette", 0))
        taux_interet_pct = int(budget.get("taux_interet_pct", 0))
        ...
        # 6) mission "charge d'intérêts" (même si tout va bien)
        # Si les intérêts prennent trop de place, l'opposition peut attaquer la gestion de dette.
        if ("OPP-M6" not in ids_existants) and interets_pct_recettes >= 8:
            nouvelles.append(mk_mission(
                mid="OPP-M6",
                axe=None,
                intitule="Réduire la facture d'intérêts",
                description=(
                    "Mettre la pression sur le cabinet : la dette coûte trop cher. "
                    "Exiger un plan crédible de réduction de la charge d'intérêts."
                ),
                priorite=2,
                critere_reussite={
                    "budget": {"interets_pct_recettes_max": 7},
                },
            ))

        # 7) mission "surplus mal utilisé" (quand le cabinet va bien)
        # surplus mais la dette ne baisse pas (delta_dette >= 0 => dette stable/augmente)
        if ("OPP-M7" not in ids_existants) and solde > 0 and delta_dette >= 0:
            nouvelles.append(mk_mission(
                mid="OPP-M7",
                axe=None,
                intitule="Dénoncer un surplus de façade",
                description=(
                    "Le cabinet affiche un excédent mais n'améliore pas la situation de la dette. "
                    "Pointer le surplus comme un coup de com' ou une mauvaise allocation."
                ),
                priorite=1,
                critere_reussite={
                    "budget": {"delta_dette_max": -1},  # dette doit baisser
                },
            ))

        # 8) mission "conjoncture favorable" (surplus + taux bas)
        # Quand le cabinet va bien, l’opposition attaque la chance et l’absence de vision.
        if ("OPP-M8" not in ids_existants) and solde > 0 and taux_interet_pct <= 3:
            nouvelles.append(mk_mission(
                mid="OPP-M8",
                axe=None,
                intitule="Montrer que le cabinet profite d'une conjoncture",
                description=(
                    "Mettre en doute le mérite du cabinet : taux bas et contexte favorable. "
                    "Exiger des investissements structurants au lieu d'accumuler des surplus."
                ),
                priorite=3,
                critere_reussite={
                    "axe": "institutionnel",
                    "min_valeur": 1,  # exemple : l'opposition veut cadrer le débat sur la gouvernance
                },
            ))
        # 9) mission "surplus sans progrès" : surplus mais score_axes n’augmente pas
        if ("OPP-M9" not in ids_existants) and solde > 0:
            # si on a un indicateur_prec, tu peux raffiner; sinon version simple:
            nouvelles.append(mk_mission(
                mid="OPP-M9",
                axe=None,
                intitule="Exiger des résultats, pas seulement un surplus",
                description=(
                    "Le cabinet est en excédent, mais les indicateurs de fond ne progressent pas. "
                    "L’opposition réclame des gains concrets sur les axes plutôt qu’un bilan comptable."
                ),
                priorite=2,
                critere_reussite={
                    "score_axes_min": score_axes + 1,  # objectif : +1 au prochain tour
                },
            ))
    ...
    def _journal_lecture_opposition(
        self,
        etat: Etat,
        indicateurs: Dict[str, Any],
        ...
        """
        Produit une lecture *politique* de l'opposition (cadre narratif / attaques / angles),
        basée sur les indicateurs et le budget.

        Objectif UI: afficher un message complémentaire à l'"Institut"' (qui est neutre).
        """
        social = int(indicateurs.get("social", 0) or 0)
        eco = int(indicateurs.get("economique", 0) or 0)
        inst = int(indicateurs.get("institutionnel", 0) or 0)
        env = int(indicateurs.get("environnement", 0) or 0)
        score_axes = int(indicateurs.get("score_axes", 0) or 0)
        solde = int(indicateurs.get("solde", 0) or 0)

        budget = indicateurs.get("budget", {}) or {}
        recettes = int(budget.get("recettes", 0) or 0)
        depenses_total = int(budget.get("depenses_total", 0) or 0)
        interets = int(budget.get("interets", 0) or 0)
        interets_pct = int(budget.get("interets_pct_recettes", 0) or 0)

        # angles: où l'opposition va frapper
        angles: List[str] = []
        if social <= 0:
            angles.append("cohésion sociale")
        if inst <= 0:
            angles.append("confiance institutionnelle")
        if eco <= 0:
            angles.append("cap économique")
        if env <= 0:
            angles.append("transition écologique")

        # les missions donnent un "script" politique (quoi marteler / quoi revendiquer)
        missions_atteintes = [m for m in missions if m.get("etat") == "atteinte"]
        missions_en_cours = [m for m in missions if m.get("etat") != "atteinte"]

        # cadrage général (quand le cabinet va bien: attaquer la durabilité / les angles morts)
        if score_axes >= 4 and solde >= 0:
            cadre = "Ils se vantent d'une embellie, mais refusent d'en partager les bénéfices."
        elif score_axes >= 4 and solde < 0:
            cadre = "Ils affichent des résultats, mais la facture s'accumule et la dette gonfle."
        elif score_axes <= 0 and solde < 0:
            cadre = "Ils échouent sur les axes et creusent le déficit: le gouvernement perd le contrôle."
        elif score_axes <= 0 and solde >= 0:
            cadre = "Ils achètent la paix sociale à court terme: le bilan reste fragile malgré le solde."
        else:
            cadre = "Le cabinet navigue à vue: aucune direction claire, et les risques s'accumulent."

        # punchlines budgétaires (sans décimales)
        punch_budget = None
        if interets_pct >= 15:
            punch_budget = f"Les intérêts avalent {interets_pct}% des recettes: c'est une dette qui gouverne."
        elif solde < 0:
            punch_budget = f"Déficit de {abs(solde)}: la dette augmente et l'espace de manœuvre se rétrécit."
        elif solde > 0 and recettes > 0:
            punch_budget = f"Surplus de {solde}: l'opposition exigera des résultats concrets pour les citoyens."

        # message final
        parts: List[str] = []
        parts.append(f"Opposition : {cadre}")

        if punch_budget:
            parts.append(punch_budget)

        if angles:
            parts.append("Angles d'attaque : " + ", ".join(angles) + ".")

        if missions_atteintes:
            # l'opposition s'attribue le mérite / revendique une victoire
            titres = [m.get('intitule', '').strip() for m in missions_atteintes if m.get('intitule')]
            if titres:
                parts.append("Ils revendiquent une première victoire : " + " / ".join(titres[:2]) + ".")

        if missions_en_cours:
            # priorités en cours (top 1-2)
            missions_en_cours = sorted(missions_en_cours, key=lambda x: int(x.get("priorite", 99) or 99))
            titres = [m.get('intitule', '').strip() for m in missions_en_cours if m.get('intitule')]
            if titres:
                parts.append("Ils martèlent : " + " / ".join(titres[:2]) + ".")
        ...
        return {
            ...
            "payload": {
                ...
                "budget": {
                    ...
                    "interets": interets,
                    "interets_pct_recettes": interets_pct,
                },
                "missions_resume": {
                    "nb_total": len(missions),
                    "nb_atteintes": len(missions_atteintes),
                    "nb_en_cours": len(missions_en_cours),
                },
            },
        }
                ...
                ui_actions=[
                    ...
                    },
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "name": "uid",
                                "label": "Identifiant unique d'engagement",
                                "required": False,
                                "domain": "uid", 
                            },
                            {
    ...
    def regle_attente_terminee(self, etat: Etat, type_attente: str) -> List[Command]:
        ...
        if type_attente == "ENGAGER_CARTE":
            ...
            if etat.programme is None:
                ...
                if etat.programme.verdict is None :
                    etat.programme.verdict = True
                    cmds.append({"op":"programme.verdict.set", "adopte" : etat.programme.verdict})
                    cmds.append({
                        "op": "journal",
                        "payload": {"message": f"Intervention du premier-ministre : {etat.programme.verdict}"},
                    })
                cmds.append({
                    "op": "journal",
